  ...
  template: `
    <section class="page">
      <div class="toolbar-row">
        <div><h1>Reviews</h1><p class="muted">Lookup records from the reviews-service.</p></div>
        <form [formGroup]="form" (ngSubmit)="load()" class="lookup">
          <mat-form-field appearance="outline"><mat-label>productId</mat-label><input matInput type="number" formControlName="id"></mat-form-field>
...
export class ReviewsComponent {
  ...
  load() {
    this.api.get<unknown[]>('reviews', `/reviews/products/${this.form.controls.id.value}`).subscribe({
